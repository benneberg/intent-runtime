...
architecture_style:
 ...
 evidence:
   ...
   - "server.ts main processor pipeline writing to events store"
 notes: ""
...
data_flow:
 value:
   ...
   - "3. Reconciliation: Extracted facts are merged with active session. facts_overridden endpoint overrides facts directly."
...
external_systems:
 ...
 confidence: HIGH
 evidence:
   - "server.ts line 34: new GoogleGenAI(...)"
 notes: "If credentials are omitted, falls back seamlessly to deterministic rule simulation."
...
extension_points:
 value:
   ...
   - "Database adapters to swap in-memory arrays for SQLite/PostgreSQL connectors"
 evidence_state: INFERRED
 confidence: HIGH
 evidence:
   - "Structure of evaluateMissingFields"
...
configuration:
 value:
   ...
   - "Port 3000 configuration mapped in code"
 evidence_state: OBSERVED
 confidence: HIGH
 evidence:
...
constraints:
 value:
   - "Single port entry (Port 3000 only) enforced by Nginx proxy container layer"
   - "Must use React 19 and Tailwind CSS v4 setup"
 evidence_state: OBSERVED
 confidence: HIGH
 evidence:
...
architecture_risks:
 value:
   ...
   - "Lack of Validation: Fact overrides bypass model confirmation, which might lead to inconsistent custom states if fields are filled with garbage."
 evidence_state: INFERRED
 confidence: HIGH
 evidence:
...
improvement_opportunities:
 value:
   - "1. Persist data: Implement a simple file-backed SQLite adapter or Firestore connector using the firebase-integration skill."
 ...
 confidence: HIGH
 evidence:
   - "Current simple types in src/types.ts and server.ts"
 notes: ""

unknown_areas:
 value:
   - "Expected behavior of the in-memory array search when event counts exceed several thousand records."
 evidence_state: OBSERVED
 confidence: MEDIUM
 evidence:
