...
idle │ ▼ awaiting_date │ ▼ awaiting_time │ ▼ awaiting_contact_information │ ▼ awaiting_confirmation │ ├── confirm ──► completed │ └── reject ──► idle
...
GEMINI_API_KEY
No
Gemini API key used for natural-language intent and fact extraction
ADMIN_API_KEY
No
Authorization token for administrative session operations
APP_URL
No
...
npm run build
```
Build the frontend and production server
```
npm start
```
...
src/App.tsx
Browser application
See ARCHITECTURE.md for how these components interact.
Design principles
...
See CONTRIBUTING.md and CODE_OF_CONDUCT.md before submitting changes.
License
Intent Runtime is licensed under the MIT License.
